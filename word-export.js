  ...
  for (let sectionIndex = 0; sectionIndex < model.sections.length; sectionIndex += 1) {
    ...
    if (section.kind === 'steps') {
      for (let stepIndex = 0; stepIndex < section.value.length; stepIndex += 1) {
        ...
        if(columns){for(let i=0;i<prepared.length;i+=2){const batch=prepared.slice(i,i+2),cells=[];for(const item of batch){cells.push(new TableCell({children:[new Paragraph({alignment:AlignmentType.CENTER,children:[new ImageRun({data:await imageBytes(item.shot.data),transformation:item.dimensions})]}),new Paragraph({alignment:AlignmentType.CENTER,children:[new TextRun({text:item.shot.caption||`Figure ${stepIndex+1}.${item.imageIndex+1}`,italics:true,color:'666666',size:18})]})]}))}if(cells.length===1)cells.push(new TableCell({children:[new Paragraph('')]}));children.push(new Table({width:{size:100,type:WidthType.PERCENTAGE},rows:[new TableRow({children:cells})]}));}}
        else{for(const item of prepared){children.push(new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:140,after:60},children:[new ImageRun({data:await imageBytes(item.shot.data),transformation:item.dimensions})]}),new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:140},children:[new TextRun({text:item.shot.caption||`Figure ${stepIndex+1}.${item.imageIndex+1}`,italics:true,color:'666666',size:18})]}));}}
