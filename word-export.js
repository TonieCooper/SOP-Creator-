export async function exportWord(model) {
  ...
  children.push(
    ...
    new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun({ text: 'Table of Contents', bold: true, color: '17365D', size: 32 })] }),
  ...
  for (let sectionIndex = 0; sectionIndex < model.sections.length; sectionIndex += 1) {
    ...
    if (section.kind === 'steps') {
      for (let stepIndex = 0; stepIndex < section.value.length; stepIndex += 1) {
        ...
        children.push(new Paragraph({ heading: HeadingLevel.HEADING_2, pageBreakBefore: !!step.pageBreak, keepNext: true, spacing: { before: 300, after: 120 }, children: [new TextRun({ text: `Step ${stepIndex + 1}: ${step.title || 'Procedure step'}`, bold: true, color: '000000', size: 28 })] }));
        if (lines(step.instruction).length) children.push(...lines(step.instruction).map((instruction) => new Paragraph({ bullet: step.mode === 'checklist' ? { level: 0 } : undefined, spacing: { after: 140, line: 320 }, children: [new TextRun({ text: step.mode === 'checklist' ? `☐ ${instruction}` : instruction, color: '000000', size: 24 })] })));
        const shots = step.screenshots || (step.image ? [{ data: step.image, caption: step.caption || '', size: 'medium' }] : []);
        const shotRuns = [];
        for (let imageIndex = 0; imageIndex < shots.length; imageIndex += 1) { const shot = shots[imageIndex]; const limits = shot.size === 'small' ? [300, 220] : shot.size === 'large' ? [560, 400] : [430, 310]; shotRuns.push({ shot, dimensions: await getImageDimensions(shot.data, limits[0], limits[1]), imageIndex }); }
        const useColumns = step.imageLayout === 'two-column' || (step.imageLayout === 'auto' && shotRuns.length > 1);
        if (useColumns) { for (let i = 0; i < shotRuns.length; i += 2) { const batch = shotRuns.slice(i, i + 2); children.push(new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: [new TableRow({ children: batch.map(({shot,dimensions,imageIndex}) => new TableCell({ children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new ImageRun({ data: await imageBytes(shot.data), transformation: dimensions })] }), new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: shot.caption || `Figure ${stepIndex + 1}.${imageIndex + 1}`, italics: true, color: '666666', size: 18 })] })] })) })] })); } }
        else { for (const {shot,dimensions,imageIndex} of shotRuns) children.push(new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 140, after: 60 }, children: [new ImageRun({ data: await imageBytes(shot.data), transformation: dimensions })] }), new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 140 }, children: [new TextRun({ text: shot.caption || `Figure ${stepIndex + 1}.${imageIndex + 1}`, italics: true, color: '666666', size: 18 })] })); }
        if ((step.check || '').trim()) children.push(new Paragraph({ spacing: { before: 100, after: 180, line: 300 }, shading: { fill: 'EAF7F1', type: ShadingType.CLEAR }, children: [new TextRun({ text: 'Completion check: ', bold: true, color: '087A55', size: 22 }), new TextRun({ text: step.check, color: '000000', size: 22 })] }));
      }
    }
  }
