  ...
  for (let sectionIndex = 0; sectionIndex < model.sections.length; sectionIndex += 1) {
    ...
    if (section.kind === 'steps') {
      for (let stepIndex = 0; stepIndex < section.value.length; stepIndex += 1) {
        ...
        const shots = step.screenshots || (step.image ? [{ data: step.image, caption: step.caption || '', size: 'medium' }] : []),shotRuns=[];
        for(let imageIndex=0;imageIndex<shots.length;imageIndex++){const shot=shots[imageIndex],limits=shot.size==='small'?[300,220]:shot.size==='large'?[560,400]:[430,310];shotRuns.push({shot,dimensions:await getImageDimensions(shot.data,limits[0],limits[1]),imageIndex})}
        const columns=step.imageLayout==='two-column'||(step.imageLayout==='auto'&&shotRuns.length>1);
        if(columns){for(let i=0;i<shotRuns.length;i+=2){const batch=shotRuns.slice(i,i+2);children.push(new Table({width:{size:100,type:WidthType.PERCENTAGE},rows:[new TableRow({children:batch.map(({shot,dimensions,imageIndex})=>new TableCell({children:[new Paragraph({alignment:AlignmentType.CENTER,children:[new ImageRun({data:await imageBytes(shot.data),transformation:dimensions})]}),new Paragraph({alignment:AlignmentType.CENTER,children:[new TextRun({text:shot.caption||`Figure ${stepIndex+1}.${imageIndex+1}`,italics:true,color:'666666',size:18})]})]}))})]}))}}
