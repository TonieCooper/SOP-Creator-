export async function exportWord(model) {
  ...
  for (let sectionIndex = 0; sectionIndex < model.sections.length; sectionIndex += 1) {
    ...
    if (section.kind === 'steps') {
      for (let stepIndex = 0; stepIndex < section.value.length; stepIndex += 1) {
        ...
        children.push(new Paragraph({ keepNext: true, spacing: { before: 300, after: 120 }, children: [new TextRun({ text: `Step ${stepIndex + 1}: ${step.title || 'Procedure step'}`, bold: true, color: '000000', size: 28 })] }));
        if (lines(step.instruction).length) children.push(...lines(step.instruction).map((instruction) => new Paragraph({ spacing: { after: 140, line: 320 }, children: [new TextRun({ text: instruction, color: '000000', size: 24 })] })));
        const shots = Array.isArray(step.screenshots) ? step.screenshots : (step.image ? [{ data: step.image, caption: step.caption || '', size: 'medium' }] : []);
        for (let imageIndex = 0; imageIndex < shots.length; imageIndex += 1) {
          const shot=shots[imageIndex],letter=String.fromCharCode(65+imageIndex),figure=`Figure ${stepIndex+1}${letter}`,limits=shot.size==='small'?[280,200]:shot.size==='large'?[540,390]:[410,295],dimensions=await getImageDimensions(shot.data,limits[0],limits[1]);
          ...
          children.push(new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:80},children:[new TextRun({text:figure,bold:true,color:'17365D',size:19}),new TextRun({text:shot.caption?`: ${shot.caption}`:'',italics:true,color:'666666',size:18})]}));
          if((shot.description||'').trim())children.push(new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:90},children:[new TextRun({text:shot.description,color:'475569',size:19})]}));
          const calls=[...new Set(((shot.editorState||{}).objects||[]).filter(o=>o.type==='callout').map(o=>Number(o.number)).filter(Number.isFinite))].sort((a,b)=>a-b);
          if(calls.length){children.push(new Paragraph({spacing:{before:60,after:40},children:[new TextRun({text:'Callout legend',bold:true,color:'17365D',size:20})]}));for(const number of calls)children.push(new Paragraph({bullet:{level:0},spacing:{after:50},children:[new TextRun({text:`${number}. ${(shot.calloutLabels||{})[number]||`Callout ${number}`}`,size:19})]}));}
        }
  ...
  const stepSection=model.sections.find(section=>section.kind==='steps');
  if(stepSection)stepSection.value.forEach((step,stepIndex)=>(step.screenshots||[]).forEach((shot,imageIndex)=>screenshotRows.push([`Figure ${stepIndex+1}${String.fromCharCode(65+imageIndex)}`,`Step ${stepIndex+1}`,shot.caption||shot.description||''])));
